export const meta = {
  name: 'jc-maths-figures',
  description: 'Extract + adversarially verify real SEC figures (scheme exemplar drawings / paper diagrams) for JC Maths diagram/graph/coordinate exhibits',
  phases: [
...
const SPECS_FILE = '/tmp/jcmth_figs.json'
...
const extracted = await parallel(CARD_IDS.map((cardId) => () =>
  agent(
    `You extract ONE real SEC figure for a JC Maths card and save it as a PNG asset. Work in ${DIR}.\n\n` +
    `YOUR CARD ID: ${cardId}\n` +
    ...
    `3. VIEW: Read the rendered PNG(s). Find the diagram/graph/coordinate-grid matching describes. If not there, render adjacent pages until you find it.\n` +
