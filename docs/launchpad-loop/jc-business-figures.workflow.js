export const meta = {
  name: 'jc-business-figures',
  description: 'Extract + adversarially verify real SEC figures (scheme exemplar drawings / paper diagrams) for JC Business Studies printed exhibits (analysed cash book, market-research table)',
  phases: [
...
const SPECS_FILE = '/tmp/jcbus_figs.json'
...
const extracted = await parallel(CARD_IDS.map((cardId) => () =>
  agent(
    `You extract ONE real SEC figure for a JC Business Studies card and save it as a PNG asset. Work in ${DIR}.\n\n` +
    `YOUR CARD ID: ${cardId}\n` +
    ...
    `3. VIEW: Read the rendered PNG(s). Find the printed exhibit (account layout / data table) matching describes. If not there, render adjacent pages until you find it.\n` +
