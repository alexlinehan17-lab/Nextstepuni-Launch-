export const meta = {
  name: 'jc-irish-figures',
  description: 'Extract + adversarially verify real SEC figures (scheme exemplar drawings / paper diagrams) for JC Irish (Gaeilge T2) OL stimulus exhibits',
  phases: [
...
const SPECS_FILE = '/tmp/jcir_figs.json'
...
const extracted = await parallel(CARD_IDS.map((cardId) => () =>
  agent(
    `You extract ONE real SEC figure for a JC Irish OL card and save it as a PNG asset. Work in ${DIR}.\n\n` +
    `YOUR CARD ID: ${cardId}\n` +
