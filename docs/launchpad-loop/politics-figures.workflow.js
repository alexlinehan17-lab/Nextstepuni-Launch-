export const meta = {
  ...
  description: 'Extract + adversarially verify real SEC figures (scheme exemplar drawings / paper diagrams) for Politics & Society cards',
  phases: [
...
const SPECS_FILE = '/tmp/pas_figs.json'
...
const extracted = await parallel(CARD_IDS.map((cardId) => () =>
  agent(
    `You extract ONE real SEC figure for a Politics & Society card and save it as a PNG asset. Work in ${DIR}.\n\n` +
    `YOUR CARD ID: ${cardId}\n` +
