export const meta = {
  name: 'jc-science-figures',
  description: 'Extract + adversarially verify real SEC figures (scheme exemplar drawings / paper diagrams) for JC Science diagram exhibits (food chain, state changes, carbon cycle)',
  phases: [
...
const SPECS_FILE = '/tmp/jcsci_figs.json'
...
const extracted = await parallel(CARD_IDS.map((cardId) => () =>
  agent(
    `You extract ONE real SEC figure for a JC Science card and save it as a PNG asset. Work in ${DIR}.\n\n` +
    `YOUR CARD ID: ${cardId}\n` +
    ...
    `3. VIEW: Read the rendered PNG(s). Find the diagram (food chain / state-change / cycle) matching describes. If not there, render adjacent pages until you find it.\n` +
