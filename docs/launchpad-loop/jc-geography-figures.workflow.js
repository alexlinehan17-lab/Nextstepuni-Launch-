export const meta = {
  name: 'jc-geography-figures',
  description: 'Extract + adversarially verify real SEC figures (scheme exemplar drawings / paper diagrams) for JC Geography exhibits (coastal photo, climate table, plate-boundary map, power-station photo, soil profile)',
  phases: [
...
const SPECS_FILE = '/tmp/jcgeo_figs.json'
...
const extracted = await parallel(CARD_IDS.map((cardId) => () =>
  agent(
    `You extract ONE real SEC figure for a JC Geography card and save it as a PNG asset. Work in ${DIR}.\n\n` +
    `YOUR CARD ID: ${cardId}\n` +
    ...
    `STEPS (use Bash + Read):\n` +
    `1. LOCATE THE PAGE: grep the txt file for the question (e.g. "grep -n 'Question' <txt>" then the part letter, or grep the topic words from describes). Count the "===PAGE n===" markers at/before the match for the 1-based PDF page. These are PAPER diagrams printed with the question (grep "Question N"); the figure is on/just after the question heading.\n` +
    `2. RENDER: ${TOOL} page <pdf> <page> /tmp/fig_${cardId}_p<page>.png 150 — render the candidate page (and a neighbour if unsure).\n` +
    `3. VIEW: Read the rendered PNG(s). Find the exhibit (photograph / climate table / map / diagram) matching describes. If not there, render adjacent pages until you find it.\n` +
