export const meta = {
  name: 'jc-history-figures',
  description: 'Extract + adversarially verify real SEC figures (scheme exemplar drawings / paper diagrams) for JC History exhibits (Red Iceberg cartoon, Soviet poster, blight map, camogie photos)',
  phases: [
...
const SPECS_FILE = '/tmp/jchist_figs.json'
...
const extracted = await parallel(CARD_IDS.map((cardId) => () =>
  agent(
    `You extract ONE real SEC figure for a JC History card and save it as a PNG asset. Work in ${DIR}.\n\n` +
    `YOUR CARD ID: ${cardId}\n` +
    ...
    `3. VIEW: Read the rendered PNG(s). Find the exhibit (cartoon / poster / map / photographs) matching describes. If not there, render adjacent pages until you find it.\n` +
