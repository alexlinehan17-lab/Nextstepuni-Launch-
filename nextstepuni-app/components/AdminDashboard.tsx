...
const StudentProgressCard: React.FC<{ user: SessionUser; userProgress: UserProgress; allCourses: CourseData[] }> = ({ user, userProgress, allCourses }) => {
  ...
  const completedCoursesCount = allCourses.reduce((count, course) => {
    const progress = userProgress[course.id];
    const isComplete = progress && progress.unlockedSection >= course.sectionsCount - 1;
    return count + (isComplete ? 1 : 0);
  }, 0);
  
  const overallProgress = totalCourses > 0 ? (completedCoursesCount / totalCourses) * 100 : 0;
  ...
  const calculateCategoryProgress = (category: CategoryType) => {
      ...
      const completedInCategory = categoryCourses.reduce((count, course) => {
          const progress = userProgress[course.id];
          const isComplete = progress && progress.unlockedSection >= course.sectionsCount - 1;
          return count + (isComplete ? 1 : 0);
      }, 0);
      
      return (completedInCategory / categoryCourses.length) * 100;
  };
