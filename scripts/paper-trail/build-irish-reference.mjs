 ...
 *
 * Import the completed metadata-only StudyClix Irish audit. The browser
 ...
 * notes, media or PDFs.
 *
 * Use --freeze-baseline exactly once before changing Irish topic tags.
 */
...
const CHECKPOINT_PATH = '/private/tmp/nextstepuni-irish-audit-progress.json';
const OUTPUT_PATH = path.join(ROOT, 'data/examTopics/irish.json');
...
const EXPECTED_VARIANTS = { higher: 25, ordinary: 26, foundation: 11 };
const EXPECTED_GROUPS = { higher: 0, ordinary: 0, foundation: 0 };
const EXPECTED_TOTALS = { reported: 1481, state: 842, mock: 563, providerSample: 76 };
...
const sourceVariants = snapshot.variants.filter(variant => variant.subjectSlug === 'irish');

if (sourceVariants.length !== 3) {
  throw new Error(`Expected 3 Irish variants, found ${sourceVariants.length}`);
}
if (checkpoint.audit && (checkpoint.captured !== 62 || checkpoint.total !== 62)) {
  throw new Error(`Irish browser audit is incomplete: ${checkpoint.captured}/${checkpoint.total}`);
}
const sourceTopics = sourceVariants.flatMap(variant => variant.topics);
if (sourceTopics.length !== 62 || Object.keys(captured).length !== 62) {
  throw new Error(
    `Irish audit must contain 62 topics; hierarchy=${sourceTopics.length}, capture=${Object.keys(captured).length}`,
  );
}
...
for (const sourceVariant of sourceVariants) {
  ...
  if (!(variant in EXPECTED_VARIANTS)) throw new Error(`Unexpected Irish variant: ${variant}`);
  ...
  const topics = sourceVariant.topics.map(sourceTopic => {
    ...
    if (
      ...
      || (audit.groupId ?? null) !== (sourceTopic.groupId ?? null)
    ) {
    ...
    return {
      id: `irish-${variant}-${sourceTopic.id}`,
...
if (JSON.stringify(totals) !== JSON.stringify(EXPECTED_TOTALS)) {
  throw new Error(
    `Irish aggregate mismatch: ${JSON.stringify(totals)} !== ${JSON.stringify(EXPECTED_TOTALS)}`,
  );
}
...
if (process.argv.includes('--freeze-baseline')) {
  ...
  if (fs.existsSync(BASELINE_PATH)) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(baseline)) {
      throw new Error('Refusing to replace the established Irish preservation baseline');
    }
  } else {
