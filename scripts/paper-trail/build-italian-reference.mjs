 ...
 *
 * Import the completed metadata-only StudyClix Italian audit. The browser
 ...
 * notes, media or PDFs.
 *
 * Use --freeze-baseline exactly once before changing Italian topic tags.
 */
...
const CHECKPOINT_PATH = '/private/tmp/nextstepuni-italian-audit-progress.json';
const OUTPUT_PATH = path.join(ROOT, 'data/examTopics/italian.json');
...
const EXPECTED_VARIANTS = { higher: 12, ordinary: 12 };
const EXPECTED_TOTALS = { reported: 340, state: 340, mock: 0, providerSample: 0 };
...
const sourceVariants = snapshot.variants.filter(variant => variant.subjectSlug === 'italian');

if (sourceVariants.length !== 2) {
  throw new Error(`Expected 2 Italian variants, found ${sourceVariants.length}`);
}
if (checkpoint.audit && (checkpoint.index !== 24 || checkpoint.total !== 24)) {
  throw new Error(`Italian browser audit is incomplete: ${checkpoint.index}/${checkpoint.total}`);
}
const sourceTopics = sourceVariants.flatMap(variant => variant.topics);
if (sourceTopics.length !== 24 || Object.keys(captured).length !== 24) {
  throw new Error(
    `Italian audit must contain 24 topics; hierarchy=${sourceTopics.length}, capture=${Object.keys(captured).length}`,
  );
}
...
for (const sourceVariant of sourceVariants) {
  ...
  if (!(variant in EXPECTED_VARIANTS)) throw new Error(`Unexpected Italian variant: ${variant}`);
  if (sourceVariant.groups.length !== 0) throw new Error(`${variant}: expected a flat topic menu`);
  ...
  const topics = sourceVariant.topics.map(sourceTopic => {
    ...
    return {
      id: `italian-${variant}-${sourceTopic.id}`,
...
if (JSON.stringify(totals) !== JSON.stringify(EXPECTED_TOTALS)) {
  throw new Error(
    `Italian aggregate mismatch: ${JSON.stringify(totals)} !== ${JSON.stringify(EXPECTED_TOTALS)}`,
  );
}
...
if (process.argv.includes('--freeze-baseline')) {
  ...
  if (fs.existsSync(BASELINE_PATH)) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(baseline)) {
      throw new Error('Refusing to replace the established Italian preservation baseline');
    }
  } else {
