 ...
 *
 * Import the metadata-only StudyClix Technology audit. The browser checkpoint
 * contains factual hierarchy, counts, source classifications, and official
 * question headings only; it contains no question text, solutions, or media.
 *
 * Use --freeze-baseline exactly once before changing Technology topic tags.
 */
...
const CHECKPOINT_PATH = '/private/tmp/nextstepuni-technology-audit-progress.json';
const OUTPUT_PATH = path.join(ROOT, 'data/examTopics/technology.json');
const TAGS_PATH = path.join(HERE, 'topic-tags/tags/technology.json');
const BASELINE_PATH = path.join(ROOT, 'test/fixtures/technologyTopicQuestionBaseline.json');

const EXPECTED_VARIANTS = { higher: 21, ordinary: 16 };
...
const sourceVariants = snapshot.variants.filter(variant => variant.subjectSlug === 'technology');

if (sourceVariants.length !== 2) {
  throw new Error(`Expected 2 Technology variants, found ${sourceVariants.length}`);
}
const sourceTopics = sourceVariants.flatMap(variant => variant.topics);
if (sourceTopics.length !== 37 || Object.keys(captured).length !== 37) {
  throw new Error(
    `Technology audit must contain 37 topics; hierarchy=${sourceTopics.length}, capture=${Object.keys(captured).length}`,
  );
}
...
for (const sourceVariant of sourceVariants) {
  ...
  if (!(variant in EXPECTED_VARIANTS)) {
    throw new Error(`Unexpected Technology variant: ${variant}`);
  }
  ...
  const topics = sourceVariant.topics.map(sourceTopic => {
    ...
    return {
      id: `technology-${variant}-${sourceTopic.id}`,
...
if (JSON.stringify(totals) !== JSON.stringify(EXPECTED_TOTALS)) {
  throw new Error(
    `Technology aggregate mismatch: ${JSON.stringify(totals)} !== ${JSON.stringify(EXPECTED_TOTALS)}`,
  );
}
...
const output = {
  ...
  reference: {
    ...
    providerSamplePolicy: 'StudyClix Sample Exams are counted for reconciliation only and excluded from NextStepUni mappings.',
  },
...
if (process.argv.includes('--freeze-baseline')) {
  ...
  if (fs.existsSync(BASELINE_PATH)) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(baseline)) {
      throw new Error('Refusing to replace the established Technology preservation baseline');
    }
  } else {
