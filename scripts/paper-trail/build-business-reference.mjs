#!/usr/bin/env node
/**
 * Import the completed, metadata-only StudyClix Business audit.
 *
 ...
 * notes, media, or PDFs.
 *
 * Use --freeze-baseline exactly once, before changing Business topic tags, to
...
const CHECKPOINT_PATH = '/private/tmp/nextstepuni-business-audit-progress.json';
const OUTPUT_PATH = path.join(ROOT, 'data/examTopics/business.json');
const TAGS_PATH = path.join(HERE, 'topic-tags/tags/business.json');
const BASELINE_PATH = path.join(ROOT, 'test/fixtures/businessTopicQuestionBaseline.json');

const EXPECTED_VARIANTS = {
  'higher-new-course': 20,
  'higher-old-course': 32,
  'ordinary-new-course': 26,
  'ordinary-old-course': 27,
};
...
const sourceVariants = snapshot.variants.filter(variant => variant.subjectSlug === 'business');

if (sourceVariants.length !== 4) {
  throw new Error(`Expected 4 Business variants, found ${sourceVariants.length}`);
}

const allSourceTopics = sourceVariants.flatMap(variant => variant.topics);
if (allSourceTopics.length !== 105 || Object.keys(captured).length !== 105) {
  throw new Error(
    `Business audit must contain 105 topics; hierarchy=${allSourceTopics.length}, capture=${Object.keys(captured).length}`,
  );
}
...
for (const sourceVariant of sourceVariants) {
  ...
  if (!(variant in EXPECTED_VARIANTS)) {
    throw new Error(`Unexpected Business variant: ${variant}`);
  }
  ...
  const topics = sourceVariant.topics.map(sourceTopic => {
    ...
    return {
      id: `business-${variant}-${sourceTopic.id}`,
  ...
  variants[variant] = {
    ...
    groups: sourceVariant.groups.map(group => ({
      id: `business-${variant}-${group.id}`,
      label: group.label,
      topicIds: group.topicIds.map(topicId => `business-${variant}-${topicId}`),
    })),
    topics,
  };
}
...
if (JSON.stringify(totals) !== JSON.stringify(EXPECTED_TOTALS)) {
  throw new Error(
    `Business aggregate mismatch: ${JSON.stringify(totals)} !== ${JSON.stringify(EXPECTED_TOTALS)}`,
  );
}
...
if (process.argv.includes('--freeze-baseline')) {
  ...
  if (fs.existsSync(BASELINE_PATH)) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(baseline)) {
      throw new Error('Refusing to replace the established Business preservation baseline');
    }
  } else {
