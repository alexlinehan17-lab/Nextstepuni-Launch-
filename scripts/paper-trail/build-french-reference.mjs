 ...
 *
 * Import the completed metadata-only StudyClix French audit. The browser
 ...
 * notes, media or PDFs.
 *
 * Use --freeze-baseline exactly once before changing French topic tags.
 */
...
const CHECKPOINT_PATH = '/private/tmp/nextstepuni-french-audit-progress.json';
const OUTPUT_PATH = path.join(ROOT, 'data/examTopics/french.json');
...
const EXPECTED_VARIANTS = { higher: 19, ordinary: 14 };
const EXPECTED_GROUPS = { higher: 0, ordinary: 0 };
const EXPECTED_TOTALS = { reported: 1244, state: 739, mock: 505, providerSample: 0 };
...
const sourceVariants = snapshot.variants.filter(variant => variant.subjectSlug === 'french');

if (sourceVariants.length !== 2) {
  throw new Error(`Expected 2 French variants, found ${sourceVariants.length}`);
}
if (checkpoint.audit && (checkpoint.index !== 33 || checkpoint.total !== 33)) {
  throw new Error(`French browser audit is incomplete: ${checkpoint.index}/${checkpoint.total}`);
}
const sourceTopics = sourceVariants.flatMap(variant => variant.topics);
if (sourceTopics.length !== 33 || Object.keys(captured).length !== 33) {
  throw new Error(
    `French audit must contain 33 topics; hierarchy=${sourceTopics.length}, capture=${Object.keys(captured).length}`,
  );
}
...
for (const sourceVariant of sourceVariants) {
  ...
  if (!(variant in EXPECTED_VARIANTS)) throw new Error(`Unexpected French variant: ${variant}`);
  ...
  const topics = sourceVariant.topics.map(sourceTopic => {
    ...
    return {
      id: `french-${variant}-${sourceTopic.id}`,
...
if (JSON.stringify(totals) !== JSON.stringify(EXPECTED_TOTALS)) {
  throw new Error(
    `French aggregate mismatch: ${JSON.stringify(totals)} !== ${JSON.stringify(EXPECTED_TOTALS)}`,
  );
}
...
if (process.argv.includes('--freeze-baseline')) {
  ...
  if (fs.existsSync(BASELINE_PATH)) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(baseline)) {
      throw new Error('Refusing to replace the established French preservation baseline');
    }
  } else {
