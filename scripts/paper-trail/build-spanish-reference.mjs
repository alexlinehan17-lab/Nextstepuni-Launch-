 ...
 *
 * Import the completed metadata-only StudyClix Spanish audit. The browser
 ...
 * notes, media or PDFs.
 *
 * Use --freeze-baseline exactly once before changing Spanish topic tags.
 */
...
const CHECKPOINT_PATH = '/private/tmp/nextstepuni-spanish-audit-progress.json';
const OUTPUT_PATH = path.join(ROOT, 'data/examTopics/spanish.json');
...
const EXPECTED_VARIANTS = { higher: 18, ordinary: 15 };
const EXPECTED_GROUPS = { higher: 3, ordinary: 3 };
const EXPECTED_TOTALS = { reported: 822, state: 471, mock: 351, providerSample: 0 };
...
const sourceVariants = snapshot.variants.filter(variant => variant.subjectSlug === 'spanish');

if (sourceVariants.length !== 2) {
  throw new Error(`Expected 2 Spanish variants, found ${sourceVariants.length}`);
}
if (checkpoint.audit && (checkpoint.index !== 33 || checkpoint.total !== 33)) {
  throw new Error(`Spanish browser audit is incomplete: ${checkpoint.index}/${checkpoint.total}`);
}
const sourceTopics = sourceVariants.flatMap(variant => variant.topics);
if (sourceTopics.length !== 33 || Object.keys(captured).length !== 33) {
  throw new Error(
    `Spanish audit must contain 33 topics; hierarchy=${sourceTopics.length}, capture=${Object.keys(captured).length}`,
  );
}
...
for (const sourceVariant of sourceVariants) {
  ...
  if (!(variant in EXPECTED_VARIANTS)) throw new Error(`Unexpected Spanish variant: ${variant}`);
  ...
  const topics = sourceVariant.topics.map(sourceTopic => {
    ...
    if (
      ...
      || audit.groupId !== sourceTopic.groupId
    ) {
      throw new Error(`${sourceTopic.sourcePath}: hierarchy/capture mismatch`);
    }
    if (audit.countMismatch || audit.unknownQuestionHeadings.length || audit.unknownSourceCards.length) {
    ...
    return {
      id: `spanish-${variant}-${sourceTopic.id}`,
...
if (JSON.stringify(totals) !== JSON.stringify(EXPECTED_TOTALS)) {
  throw new Error(
    `Spanish aggregate mismatch: ${JSON.stringify(totals)} !== ${JSON.stringify(EXPECTED_TOTALS)}`,
  );
}
...
if (process.argv.includes('--freeze-baseline')) {
  ...
  if (fs.existsSync(BASELINE_PATH)) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(baseline)) {
      throw new Error('Refusing to replace the established Spanish preservation baseline');
    }
  } else {
