...
"""Author every Computer Science part the scheme states and prices.
    ...
    python3 scripts/markbank/authoring/computer_science_all.py            # emit JSON
    python3 scripts/markbank/authoring/computer_science_all.py --report   # counts and refusals
...
Three refusals, all of them the rules already in force:
  ...
  * it prints no tariff that reads one way -- see computer_science_scheme.tariff(), which
...
from computer_science_lib import Author, Refused            # noqa: E402
from computer_science_topics import topic_for, concept_for                 # noqa: E402
import cardlint                                             # noqa: E402
import computer_science_question_figures as CSF                           # noqa: E402
...
# numbering continues, but a continuation stem at the join can otherwise absorb
# rough-work pages, acknowledgements and the whole Section C cover.  None of
# that is question content.  Cut it at the first unmistakable piece of booklet
# furniture and keep any genuine setup which precedes it.
BOOKLET_FURNITURE = re.compile(
    r'\b(?:Space for extra work\.\s*Indicate clearly the number and part of the '
    r'question\(s\) you are answering\.|Acknowledgements\s+(?:Images|Texts)|'
    r'Copyright notice|Computer Science Section C (?:Higher|Ordinary) Level|'
    r'Section C Programming\s+\d+ marks)', re.I)
PAGE_FURNITURE = re.compile(
    r'\bThis question continues on the next page\.\s*|'
    r'\bDo not hand this paper up\b', re.I)


def clean_booklet_furniture(card):
    """Remove material from adjacent SEC pages without rewriting the ask."""
    question = ' '.join(PAGE_FURNITURE.sub('', card.get('questionText', '')).split())
    if question:
    ...
    if not stem:
        return
    match = BOOKLET_FURNITURE.search(stem)
    if match:
        stem = stem[:match.start()]
    stem = ' '.join(PAGE_FURNITURE.sub('', stem).split()).strip()

    # When an inspected crop carries Question 15's code/table/diagram, the
    # flattened prefix is a second, less legible copy of the same printed
    # matter.  Its actual subpart prompt remains in questionText.
    if card.get('figureKey') and re.search(r'\bQ15(?:\b|\()', card['questionRef']):
        stem = ''

    # The generic reader sometimes files a complete one-part question as both
    # stem and questionText, with only a trailing "Figure N" added to the stem.
    # Showing the same paragraph twice is not context.
    if stem:
        stem_without_label = re.sub(r'\s+Figure\s+\d+\s*$', '', stem, flags=re.I)
        if ' '.join(stem_without_label.split()) == ' '.join(card['questionText'].split()):
            stem = ''
...
def cardable(points):
    ...
    well, never what, which is the same line computer_science_scheme.bands() draws.
