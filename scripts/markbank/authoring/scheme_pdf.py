#!/usr/bin/env python3
"""Marking schemes, any subject, read from the PDF — the table repair.
...
# Subjects head a scheme question every way there is: "Q1 (a)" in Agricultural
# Science, "QUESTION 1 (a)" in Chemistry, a bare "1" in Physics. The bare form
# is anchored on what follows so it cannot swallow a numbered rubric line.
QHEAD = re.compile(r'^(?:QUESTION\s+(\d{1,2})\b|Q\s?(\d{1,2})\b|(\d{1,2})[.)]?\s+(?=\(|[A-Z]))\s*')
...
class SchemePdf:
    ...
    def __init__(self, subject, year, level):
        ...
        for text in self._blocks():
            ...
            rest = text
            if head:
                q = int(next(g for g in head.groups() if g))
                rest = text[head.end():]
