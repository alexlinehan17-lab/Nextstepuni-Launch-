...
# "(a)" at Higher Level, "Part (a)" at Ordinary, and "6(c) -" in the 2017
# indicative scheme.  The last form still opens a new part: treating it as an
# answer line made Q6(c) disappear and bled its content into Q6(b).
PART = re.compile(
    r'^(?:(?:Part\s+)?\((?P<paren>[a-h])\)|\d{1,2}\((?P<numbered>[a-h])\))'
    r'\s*[-–:]?\s*(?P<text>.*)$', re.I)


def part_match(line):
    """Return ``(letter, trailing text)`` for every printed part-heading form."""
    match = PART.match(line)
    if not match:
        return None
    return (match.group('paren') or match.group('numbered'), match.group('text'))
...
FURNITURE = re.compile(
    ...
    r'|Scrúdú\s+Ardteistiméireachta|_{20,}\s*$'
        ...
        if m and not (m.group('rest') and not PART.match(m.group('rest').strip())
            ...
            if rest:
                ...
                if parsed_part:
                    ...
                    if part_text.strip():
                        out[(q, letter)].append(part_text.strip())
            continue
