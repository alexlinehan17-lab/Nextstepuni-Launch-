...
export function useMockResults(uid: string | undefined) {
  ...
  useEffect(() => {
    ...
    if (reconciled.length > 0 && JSON.stringify(canonical) !== JSON.stringify(reconciled)) {
      if (isDemo) {
        updateDemoProgress(current => ({ ...current, ...mockResultsStoragePatch(reconciled) }));
      } else {
    ...
    setIsLoaded(true);
  }, [uid, progressLoaded, rawProgressDoc, isDemo, updateDemoProgress]);

  const addMockResult = useCallback((mock: Omit<UnifiedMockResult, 'id' | 'timestamp'>) => {
    if (!uid || mock.date > toDateKey(new Date())) return;
    const newMock: UnifiedMockResult = {
      ...mock,
      resultKind: resolveMockResultKind(mock),
    ...
    setMocks(next);
    if (isDemo) {
      updateDemoProgress(current => ({ ...current, ...mockResultsStoragePatch(next) }));
    } else {
      setDoc(doc(db, 'progress', uid), mockResultsStoragePatch(next), { merge: true })
        .catch((e) => reportSaveError('useMockResults.save', e));
    }
  ...
  const removeMockResult = useCallback((id: string) => {
    ...
    if (isDemo) {
      updateDemoProgress(current => ({
        ...current,
        ...mockResultsStoragePatch(next),
        pointsPassport: { ...(current.pointsPassport ?? {}), mockResults: [] },
        warRoom: { ...(current.warRoom ?? {}), mockResults: [] },
      }));
    } else {
      setDoc(doc(db, 'progress', uid), mockResultsDeletionPatch(next), { merge: true })
        .catch((e) => reportSaveError('useMockResults.save', e));
    }
  }, [uid, isDemo, updateDemoProgress]);
