 ...
 *  - `questionTotal` — the scheme prices the whole question but does not state
 *    how those marks divide across its parts. Rows therefore carry no invented
 *    values while the card still reports the official total.
 *
...
export const rowCapFor = (kind: TariffModel['kind']): number =>
  kind === 'questionTotal' ? MAX_LONG_OPTION_ROWS
