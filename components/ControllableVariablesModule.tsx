...
import { CONTROLLABLE_VARIABLES_REFERENCE_LIST } from '../data/references/controllableVariables';
...
const ControllableVariablesModule: React.FC<{ onBack: () => void; progress: ModuleProgress; onProgressUpdate: (progress: ModuleProgress) => void }> = ({ onBack, progress, onProgressUpdate }) => {
  ...
  const sections = essentials ? [
    ...
    { id: 'sleep', title: 'Sleep & Nutrition', eyebrow: '02 // The Foundation', icon: Moon },
    { id: 'exercise', title: 'Exercise & The Cycle', eyebrow: '03 // The Catalyst', icon: Zap },
    { id: 'blueprint', title: 'The High-Performance Blueprint', eyebrow: '04 // Your Protocol', icon: ClipboardCheck },
  ] : [
    { id: 'engine', title: 'The Performance Engine', eyebrow: '01 // The Core Idea', icon: Cpu },
        ...
        <>
          {activeSection === 0 && (
            <ReadingSection title="The Performance Engine." eyebrow="Step 1" icon={Cpu} theme={theme}>
              {essentials ? (
                <>
                  <p>Your brain is a high-performance engine. How well you learn depends on three inputs: <Highlight description="This is basically the save button for everything you study. Without proper sleep, your brain can't hold onto what you learned." theme={theme}>Sleep</Highlight>, <Highlight description="Your brain's fuel. The right food keeps you focused; the wrong food leaves you in a fog." theme={theme}>Nutrition</Highlight>, and <Highlight description="Moving your body actually makes your brain better at learning. It's not just about fitness." theme={theme}>Exercise</Highlight>.</p>
                  <p>When you're tired or hungry, your <Highlight description="This is the part of your brain that lets you hold a tricky Maths problem in your head or plan out an English essay. It's like the boss of your brain — but it's still developing in your teens, which makes it powerful but also easy to throw off." theme={theme}>Prefrontal Cortex</Highlight><Cite n={1} /> disconnects from your emotional brain.<Cite n={2} /> You can't think straight. Focus on <Highlight description="Instead of just trying to study more hours, focus on the things that give you more energy and sharper focus — sleep, food, and exercise. That's what actually decides how good your study sessions are." theme={theme}>energy management</Highlight>, not just time management.</p>
                </>
              ) : (
                <>
            ...
            <ReadingSection title={essentials ? "Sleep & Nutrition." : "The Save Button: Sleep."} eyebrow="Step 2" icon={Moon} theme={theme}>
              {essentials ? (
                ...
                <>
                  ...
                  <p>Here's how bad it gets. After just a week of 5-hour nights, your working memory takes a real hit — chronic short sleep builds up a cognitive deficit that keeps growing the longer it drags on.<Cite n={4} /> And there's something even starker: <Highlight description="After being awake for 17-19 hours straight, your brain works about as well as if you'd been drinking. So if you got up at 6am, by late evening your thinking is seriously impaired." theme={theme}>the drunk comparison</Highlight>. After about 17 hours awake, your brain is roughly as impaired as if you had a blood alcohol level of 0.05% — and it keeps deepening the longer you stay up.<Cite n={5} /> If you wake up at 6 AM, by late evening you're basically studying drunk. And "catching up at the weekend" doesn't really fix it — two nights of extra sleep isn't enough to undo the damage.</p>
                  <CognitiveImpairmentClock />
                </>
              )}
            </ReadingSection>
          ...
          {(essentials ? activeSection === 3 : activeSection === 5) && (
            <ReadingSection title="The High-Performance Blueprint." eyebrow={essentials ? "Step 4" : "Step 6"} icon={ClipboardCheck} theme={theme}>
              {essentials ? (
                <p>Here's what a great study day looks like. This checklist works with your teenage brain, not against it. Use it on study days at home.</p>
              ) : (
                <>
