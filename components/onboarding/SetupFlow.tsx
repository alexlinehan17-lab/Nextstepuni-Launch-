...
export default function SetupFlow({ userId, userName, onComplete, onSkip, mode = 'fresh', transitionTargetYear }: OnboardingProps) {
  ...
  const title = ({ welcome: 'Make your mark.', year: 'Where are you now?', north: 'What drives you?', vision: 'Picture your future.', subjects: 'Make it yours.', grades: currentSubject || 'Your grades.', schedule: 'Make room for progress.', summary: `You’re ready, ${userName.trim().split(/\s+/)[0] || 'let’s go'}.` })[draft.step];
  ...
  return <div className="setup-flow" data-surface={surface} data-prevent-pull-to-refresh="true" data-reduced-motion={Boolean(reduced)}>
    <div ref={scroller} className="setup-scroller" data-testid="onboarding-scroll-region">
      <div className="setup-shell">
        ...
        <main className="setup-content" key={`${draft.step}-${draft.step === 'grades' ? currentSubject : ''}`}>
          ...
          {draft.step === 'north' && <><p className="setup-description">Not what you “should” want. What actually matters to you?</p><div className="setup-grid">{categories.map(item => <button type="button" className="setup-choice setup-motivation" key={item.id} aria-pressed={draft.category === item.id} onClick={() => { patch({ category: item.id }); setVisionGroup(item.id); }}><img src={NORTH_STAR_CATEGORY_BLOBS[item.id].iconPath} alt="" /><span>{item.label}</span></button>)}</div><p className="setup-description setup-selected-description" aria-live="polite">{category?.description || 'Choose your own reason to keep going.'}</p></>}
