...
export const ChoiceControl: React.FC<ChoiceControlProps> = ({
  ...
  <button
    ...
    className={`group inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border-2 text-left font-medium transition-[background-color,border-color,color,box-shadow,transform] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(var(--accent),0.38)] focus-visible:ring-offset-2 ${
      ...
      selected
        ? 'border-[#1A1A1A] bg-[#F26B1F] text-white shadow-[4px_4px_0_0_#1A1A1A] hover:-translate-y-0.5 hover:shadow-[5px_5px_0_0_#1A1A1A] active:translate-x-1 active:translate-y-1 active:shadow-none'
        : 'border-[#E5E1DB] bg-white text-[var(--text-body)] hover:border-[#1A1A1A] hover:bg-zinc-50 active:translate-y-px dark:border-zinc-700 dark:bg-zinc-900 dark:hover:bg-zinc-800'
    } ${className}`}
    style={style}
    {...rest}
  >
    {(markerClassName || markerColor) && (
      <span
        ...
        style={selected
          ? { backgroundColor: 'transparent', boxShadow: 'inset 0 0 0 1.5px #FFFFFF' }
    ...
    {icon && (
      <span
        aria-hidden="true"
        className={`shrink-0 ${selected ? 'text-white' : 'text-[var(--text-muted)]'}`}
      >
        {icon}
      </span>
    )}
