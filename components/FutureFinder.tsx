...
function DetailPhase({
  ...
  return (
    <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.35 }}>
      ...
      <div className="bg-white dark:bg-zinc-900 overflow-hidden" style={{ border: '1px solid var(--outline-soft)', borderRadius: '12px' }}>
        {/* Header */}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {c.subjectBonus.map(sub => (
                <span key={sub} className="px-3 py-1.5 rounded-lg text-sm font-medium bg-white dark:bg-zinc-900" style={{ color: COLORS.accent }}>{sub}</span>
              ))}
            </div>
          </div>
    ...
    <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="space-y-8">
      ...
      {alternates.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {alternates.map(alt => (
              <div key={alt.cluster.id} className="rounded-xl p-4 bg-white dark:bg-zinc-900" style={{ border: '1px solid var(--outline-soft)' }}>
      ...
      <div className="rounded-xl p-4 bg-white dark:bg-zinc-900" style={{ border: '1px solid var(--outline-soft)' }}>
  ...
  return createPortal(
    ...
    >
      <MotionDiv
        ...
        className="bg-white dark:bg-zinc-900 rounded-2xl border-2 border-[#1A1A1A] dark:border-zinc-700 shadow-[6px_6px_0_0_#1A1A1A] dark:shadow-[6px_6px_0_0_#3f3f46] max-w-xl w-full max-h-[90vh] flex flex-col overflow-hidden"
