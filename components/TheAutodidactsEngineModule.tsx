        ...
        <>
          {activeSection === 0 && (
            <ReadingSection title="Not All Study Is Equal." eyebrow="Step 1" icon={SlidersHorizontal} theme={theme}>
              {essentials ? (<><p>Most study is <Highlight description="Going through the motions without really thinking -- like re-reading your notes or highlighting a textbook. It feels productive, but you're not actually getting better at anything." theme={theme}>passive</Highlight> and wasted. You need <Highlight description="Practising with a clear goal, full focus, and -- this is the key part -- a way to check whether you're getting it right. It's harder, but it's what actually makes you improve." theme={theme}>focused practice</Highlight><Cite n={1} /> with a feedback loop. Check your work against the right answer.<Cite n={2} /> This module shows you how to be your own teacher.</p></>) : (<><p>Most study time is wasted. It's spent on what we call <Highlight description="Going through the motions without really thinking -- like re-reading your notes or highlighting a textbook. It feels productive, but you're not actually getting better at anything." theme={theme}>Passive Practice</Highlight>--just going through the motions. This feels productive, but it doesn't build skill. To actually get better, you need <Highlight description="Practising with a clear goal, full focus, and -- this is the key part -- a way to check whether you're getting it right. It's harder, but it's what actually makes you improve." theme={theme}>Focused Practice</Highlight>.<Cite n={1} /></p>
              <p>The key ingredient of focused practice is a good feedback loop -- a way to check your work against the right answer.<Cite n={2} /> A great teacher does this for you automatically. But when you're studying alone, you don't have that. The goal of this module is to show you how to become your own teacher by building that feedback into your study sessions.</p></>)}
            </ReadingSection>
          )}
           {activeSection === 1 && (
            <ReadingSection title="Two Rules for Self-Study." eyebrow="Step 2" icon={Repeat} theme={theme}>
              {essentials ? (<><p>Two rules for self-study. First, notice your mistakes. Compare your work to the right answer.<Cite n={3} /> Second, do the work and check the work separately. Your brain cannot handle both at once.<Cite n={4} /> Do step one, then step two.</p></>) : (<><p>To build your own feedback loop, you need to follow two rules about how your brain works. First, <Highlight description="Your brain learns best when you actually notice your mistakes. If you just skip past errors, you don't learn from them. You need to compare your work against the correct answer so mistakes are impossible to ignore." theme={theme}>Pay Attention to Your Mistakes</Highlight>: Your brain learns the most when it spots an error. If you just gloss over mistakes, nothing sticks. That's why you need to compare your work against a correct version -- a marking scheme, a worked solution, a model answer -- so mistakes become impossible to ignore.<Cite n={3} /></p>
              <p>Second, <Highlight description="Your brain can only hold a few things at once. If you try to do the work AND check the work at the same time, you'll do both badly. Do them separately instead." theme={theme}>Don't Do Everything at Once</Highlight>: Your brain can only juggle so much at one time.<Cite n={4} /> You can't do the work and check the work at the same time -- you'll do both badly. This <Highlight description="When your brain tries to handle two demanding tasks at the same time, both suffer. It's like trying to have two conversations at once -- neither one goes well." theme={theme}>mental overload</Highlight> is why checking your work as you go feels so hard. The fix is simple: use a <Highlight description="A two-step process: first you do the work, then you check the work. Keeping these separate means you can give each one your full attention." theme={theme}>two-step process</Highlight> -- first do the work, then check the work. Keep them separate.</p></>)}
            </ReadingSection>
          )}
           {activeSection === 2 && (
            <ReadingSection title="The Maths Loop." eyebrow="Step 3" icon={Brain} theme={theme}>
              {essentials ? (<><p>For Maths, use the <Highlight description="You cover the worked solution, try just the first step yourself, then uncover the first step of the solution to compare. You go one step at a time so you get instant feedback on every single line." theme={theme}>Split-Page Method</Highlight>. Cover the solution. Try one step. Uncover and compare. When your answer differs, write down why in your own words.<Cite n={5} /> That is where real learning happens.</p></>) : (<><p>In Maths, your "stand-in teacher" is a full worked solution or marking scheme. The technique is the <Highlight description="You cover the worked solution, try just the first step yourself, then uncover the first step of the solution to compare. You go one step at a time so you get instant feedback on every single line." theme={theme}>Split-Page Method</Highlight>. You cover the solution, try just one step of the problem, stop, then uncover the correct first step. This gives you instant feedback on every single line of working.</p>
              <p>If your step matches, keep going. If it doesn't -- that's where the real learning happens. Write down in your own words why the correct step was different from yours. This is called <Highlight description="Explaining something to yourself in your own words. It sounds simple, but it forces you to actually understand the 'why' behind each step instead of just copying it." theme={theme}>self-explanation</Highlight><Cite n={5} /> -- and it forces you to actually understand the 'why' behind each step, instead of just copying and moving on.</p></>)}
              <SplitPageSimulator />
            </ReadingSection>
          )}
          {activeSection === 3 && (
            <ReadingSection title="The Language Loop." eyebrow="Step 4" icon={BookOpen} theme={theme}>
              {essentials ? (<><p>For languages, use <Highlight description="You translate a passage from Irish (or French, etc.) into English, wait a while, then translate your English version back without looking at the original. Then you compare the two versions side by side to spot your mistakes." theme={theme}>Back-Translation</Highlight>. Translate a passage to English. Wait an hour. Translate it back. Compare with the original. Every difference shows you a gap in your understanding.</p></>) : (<><p>For languages, your "stand-in teacher" is a correct text in the language you're learning. The technique is <Highlight description="You translate a passage from Irish (or French, etc.) into English, wait a while, then translate your English version back without looking at the original. Then you compare the two versions side by side to spot your mistakes." theme={theme}>Back-Translation</Highlight>. You take a short passage in Irish (or French, etc.), translate it into English, wait an hour, then translate your English version <em>back</em> into the original language without looking at it. Finally, you compare your version with the original, side by side.</p>
