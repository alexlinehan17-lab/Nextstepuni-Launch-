...
const Shell: React.FC<{ stage: Stage; back?: () => void; children: React.ReactNode }> = ({ stage, back, children }) => {
  ...
  return <div className="immersive-deck-theme w-full max-w-6xl mx-auto pb-16 px-3 sm:px-5">
    {stage === 'priorities' && <ToolHero toolId="your-possible-life" eyebrow="Understand · Career discovery" title="Your Possible Life" subtitle="Possibilities, not predictions." />}
    <header className="flex items-center justify-between gap-5 py-5 border-b" style={{ borderColor: HAIRLINE }}>
      <div className="flex items-center gap-4 min-w-0">
        ...
        {stage !== 'priorities' && <div><div className="text-[11px] tracking-[.24em] uppercase font-bold" style={{ color: ACCENT_TEXT }}>Your Possible Life</div><div className="text-sm" style={{ color: MUTED }}>Possibilities, not predictions.</div></div>}
      </div>
    ...
    <AnimatePresence mode="wait"><motion.div key={stage} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: .32, ease: [0.22, 1, 0.36, 1] }}>{children}</motion.div></AnimatePresence>
  </div>;
};
...
const YourPossibleLife: React.FC<{ uid?: string; profile: StudentSubjectProfile }> = ({ uid, profile }) => {
  ...
  if (stage === 'priorities') return <Shell stage={stage}><section className="py-12 sm:py-20 grid lg:grid-cols-[.8fr_1.2fr] gap-10 lg:gap-20 items-start"><div><div className="font-mono text-sm font-bold mb-5" style={{ color: ACCENT_TEXT }}>01 — BEFORE THE JOB TITLE</div><h2 className="text-5xl sm:text-7xl leading-[.94]" style={{ fontFamily: SERIF, color: INK }}>What should your future make room for?</h2><p className="mt-6 text-lg leading-relaxed max-w-lg" style={{ color: MUTED }}>Choose three to five things. There is no perfect combination, and we will not turn it into a personality score.</p></div><div className="grid sm:grid-cols-2 gap-3">{LIFE_PRIORITIES.map((priority, index) => { const active = priorities.includes(priority); return <button key={priority} onClick={() => togglePriority(priority)} className="min-h-[82px] rounded-2xl px-5 flex items-center justify-between text-left transition-transform active:scale-[.99]" style={{ border: `2px solid ${active ? INK : HAIRLINE}`, background: active ? CREAM : PAPER, color: INK }}><span><span className="font-mono text-[11px] mr-3" style={{ color: ACCENT_TEXT }}>{String(index + 1).padStart(2, '0')}</span><span className="text-lg font-bold">{PRIORITY_LABELS[priority]}</span></span>{active && <Check size={20} color={ACCENT_TEXT}/>}</button>; })}<div className="sm:col-span-2 flex items-center justify-between mt-5"><span className="text-sm" style={{ color: MUTED }}>{priorities.length}/5 chosen</span><Button disabled={priorities.length < 3} onClick={() => { persist({ priorities, selectedCareerIds: possibilities.map((item) => item.id) }); setStage('possibilities'); }}>Meet three possibilities <ArrowRight className="inline ml-2" size={18}/></Button></div></div></section></Shell>;

  if (stage === 'possibilities') return <Shell stage={stage} back={() => setStage('priorities')}><section className="py-12"><div className="max-w-3xl mb-10"><div className="font-mono text-sm font-bold mb-4" style={{ color: ACCENT_TEXT }}>THREE DOORS, NOT A PODIUM</div><h1 className="text-4xl sm:text-6xl" style={{ fontFamily: SERIF, color: INK }}>Different futures can protect the same things.</h1><p className="mt-4 text-lg" style={{ color: MUTED }}>One close match, one adjacent possibility and one route you may not have considered. None is labelled “best”.</p></div><div className="grid lg:grid-cols-3 gap-5">{possibilities.map((item, index) => <article key={item.id} className="rounded-[28px] p-6 flex flex-col min-h-[520px]" style={{ background: PAPER, border: `2px solid ${INK}`, boxShadow: `7px 8px 0 ${INK}` }}><div className="font-mono text-sm font-bold" style={{ color: ACCENT_TEXT }}>0{index + 1}</div><div className="my-8 flex justify-center"><CareerArtwork career={item}/></div><h2 className="text-3xl" style={{ fontFamily: SERIF, color: INK }}>{item.title}</h2><p className="mt-2 min-h-[52px]" style={{ color: MUTED }}>{item.tagline}</p><div className="mt-5 pt-5 border-t text-sm leading-relaxed flex-1" style={{ borderColor: HAIRLINE, color: INK }}>{item.whatYouDo[0]}</div><Button aria-label={`Explore a day as ${item.title}`} onClick={() => choose(item.id)} className="w-full mt-6">Step inside <ArrowRight className="inline ml-2" size={17}/></Button></article>)}</div></section></Shell>;
