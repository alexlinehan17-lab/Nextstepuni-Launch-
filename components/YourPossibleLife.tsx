...
const YourPossibleLife: React.FC<{ uid?: string; profile: StudentSubjectProfile }> = ({ uid, profile }) => {
  ...
  if (stage === 'priorities') return <Shell stage={stage}><section className="lp-values-layout"><div><p className="lp-eyebrow">Before the job title</p><h1>What should your future make room for?</h1><p className="lp-body mt-5">Choose three to five things that matter to you. There’s no perfect combination.</p></div><div><div className="lp-values-grid">{LIFE_PRIORITIES.map((priority,index) => <button key={priority} aria-pressed={priorities.includes(priority)} onClick={() => togglePriority(priority)}><span>{String(index+1).padStart(2,'0')}</span><strong>{PRIORITY_LABELS[priority]}</strong></button>)}</div><div className="lp-sticky-action lp-action-row"><span className="text-sm" aria-live="polite">{priorities.length}/5 chosen</span><button className="lp-button disabled:opacity-40" disabled={priorities.length < 3} onClick={() => {persist({priorities,selectedCareerIds:possibilities.map(item => item.id)});setStage('possibilities');}}>Meet three possibilities <ArrowRight size={17} /></button></div></div></section></Shell>;
