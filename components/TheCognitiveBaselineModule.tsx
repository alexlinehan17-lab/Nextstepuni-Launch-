...
const CognitiveBaselineChecklist = () => {
    ...
    return(
        <div className="my-10 p-8 md:p-12 bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700">
             <h4 className="font-serif text-2xl font-semibold text-zinc-800 dark:text-white text-center">My Cognitive Baseline Checklist</h4>
             ...
             <div className="space-y-4">
                {items.map(item => (
                    <div key={item.key} onClick={() => toggleCheck(item.key)} className={`p-4 rounded-xl border flex items-center gap-4 cursor-pointer transition-all ${checks[item.key] ? 'bg-emerald-50 border-emerald-300' : 'bg-zinc-50 border-zinc-200 dark:border-zinc-700'}`}>
