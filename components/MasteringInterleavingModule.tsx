        ...
        <>
          {activeSection === 0 && (
            <ReadingSection title="Why 'Finishing a Topic' Fools You." eyebrow="Step 1" icon={Layers} theme={theme}>
              ...
              <p>Here's the problem: your brain isn't being trained for the real Leaving Cert. The exam doesn't hand you 20 algebra questions in a row. It throws a mix at you. When you practise the same type of question over and over, you never have to figure out *what* to do, only *how* to do it. You never build the skill of <Highlight description="Being able to look at a question and figure out which approach to use -- like knowing whether you need the chain rule or the product rule." theme={theme}>telling similar problems apart</Highlight>, and that's exactly what the exam tests.</p>
            </ReadingSection>
          )}
           {activeSection === 1 && (
            <ReadingSection title="The 'Topic Salad' Method." eyebrow="Step 2" icon={Shuffle} theme={theme}>
              ...
              <p>This works at two levels. <Highlight description="Mixing different subjects across your week -- like doing Maths, English, and Biology all on Monday evening instead of just one." theme={theme}>Mixing across subjects</Highlight> is about your weekly schedule: instead of one subject for 3 hours, study three subjects for 1 hour each. <Highlight description="Mixing different topics within one subject session -- like doing a bit of Algebra, then Stats, then Geometry, instead of just Algebra." theme={theme}>Mixing within a subject</Highlight> is about shuffling topics *within* a single study session. This one is harder, but that extra effort is exactly where the real learning happens.</p>
