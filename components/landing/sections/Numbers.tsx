 ...
 * the white page, four numerals at display size that roll into place like a
 * mechanical counter as they arrive (NumberFlow; see fx-c/Odometer). Every
 * figure is computed from repo data (see demoData.ts header). This is the
 * page's one dark surface.
 */

import React from 'react';
import { COPY } from '../copy';
import Odometer from '../fx-c/Odometer';
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-12 gap-x-8">
        {COPY.numbers.items.map((n, i) => (
          <Reveal key={n.label} delay={i * 0.08}>
            <div style={{ fontFamily: FONT.serif, fontWeight: 600, fontSize: 'clamp(52px, 7.4vw, 112px)', lineHeight: 0.92, letterSpacing: '-0.04em', color: L.paper, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
              <Odometer value={n.value} delay={i * 0.08} />
            </div>
