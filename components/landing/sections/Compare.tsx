 ...
 *
 * Compare — one framed document. Drag the hairline divider and the real SEC
 * exam question (left) gives way to the marking scheme's row for the same
 * part (right), with the marks column that scores it. Both crops are the
 * SEC's own pages — Economics 2024, Higher Level, Question 1, from the paper
 * and its published scheme — exported at the same pixel size so the
 * infographic, the (a) prompt and the workings box sit on top of each other.
 * A 1px ink handle with a 10px orange grip is the only chrome; the handle
 * is a keyboard slider (arrow keys) with a 44px hit area.
 */

import React from 'react';
import { ReactCompareSlider } from 'react-compare-slider';
import { Body, Container, Display, Eyebrow, Frame, SectionRule } from '../primitives';
...
const Compare: React.FC = () => (
  <section id="compare" className={SPACE.sectionTight} style={{ position: 'relative', scrollMarginTop: 70 }}>
    <SectionRule />
    <Container>
        ...
        <div className="lg:col-span-8 min-w-0">
          <Frame title={PAIR.title} meta="Paper Trail">
            <div className="fxc-compare">
              <ReactCompareSlider
                itemOne={<Side src={PAIR.paper} alt={PAIR.paperAlt} tag={TEXT.left} edge="left" />}
                itemTwo={<Side src={PAIR.scheme} alt={PAIR.schemeAlt} tag={TEXT.right} edge="right" />}
                handle={<InkHandle />}
                defaultPosition={55}
                keyboardIncrement="5%"
                onlyHandleDraggable
