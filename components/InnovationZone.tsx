...
const InnovationZone: React.FC<InnovationZoneProps> = ({ onBack, user, initialSubjectProfile, savedJourneyResult, onJourneyComplete, settings: _settings, updateSetting: _updateSetting, onCosmeticUnlocksChange, onStudyNow, dismissedGuides: _dismissedGuides, onDismissGuide: _onDismissGuide }) => {
  ...
  return (
    <div
      className={`${activeTool === 'paper-trail' ? 'paper-trail-host ' : ''}product-shell launchpad-shell ${mobileAppDesign && !WIP_TOOL_IDS.has(activeTool ?? '') ? 'mobile-brand-tools' : ''} min-h-screen bg-[var(--surface-canvas)] transition-colors duration-500 overflow-x-hidden relative flex flex-col items-center pb-36 md:pb-24`}
            ...
            {!activeTool ? (
                ...
                >
                    ...
                    {!mobileAppDesign && filteredTools.length === 0 && curriculumLevel === 'junior' && (
                      <div className="rounded-2xl p-10 text-center" style={{ backgroundColor: 'var(--surface-paper)', border: '2px solid #1A1A1A' }}>
