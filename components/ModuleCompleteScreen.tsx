          ...
          <div className="min-h-screen flex flex-col items-center justify-center px-6 py-16">
            <div className="max-w-md w-full text-center">
              ...
              <MotionDiv initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: d(4), duration: 0.5 }}>
                <h1 className="font-serif font-bold text-zinc-900 dark:text-white" style={{ fontSize: 'clamp(28px, 8vw, 44px)', lineHeight: 1.15, letterSpacing: '-0.025em' }}>{moduleTitle}</h1>
              </MotionDiv>

              {moduleSubtitle && <MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: d(6), duration: 0.4 }}><p className="text-base text-zinc-500 dark:text-zinc-400 mt-2">{moduleSubtitle}</p></MotionDiv>}
              ...
              {modulesCompleted !== undefined && totalModules !== undefined && (
                <MotionDiv initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: d(8), duration: 0.5 }} className="flex justify-center gap-4 mt-8">
                  <div className="px-6 py-4 text-center" style={{ backgroundColor: '#93C5FD', border: '2.5px solid #2563EB', borderRadius: 16, boxShadow: '3px 3px 0px 0px #2563EB' }}>
                    <div className="flex items-center justify-center gap-1.5 mb-1"><BookOpen size={14} style={{ color: '#1E3A8A' }} /><span className="text-xl font-bold" style={{ color: '#1E3A8A' }}>{modulesCompleted}/{totalModules}</span></div>
                  ...
                  <div className="px-6 py-4 text-center" style={{ backgroundColor: '#FCD34D', border: '2.5px solid #D97706', borderRadius: 16, boxShadow: '3px 3px 0px 0px #D97706' }}>
                    <div className="flex items-center justify-center gap-1.5 mb-1"><Layers size={14} style={{ color: '#78350F' }} /><span className="text-xl font-bold" style={{ color: '#78350F' }}>{sectionsCount}</span></div>
              ...
              <MotionDiv initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: d(14), duration: 0.5 }} className="mt-8 space-y-3">
                <motion.button onClick={onContinue} className="w-full py-4 rounded-2xl text-base font-bold text-white flex items-center justify-center gap-2" style={{ backgroundColor: COLORS.success, border: `2.5px solid ${COLORS.successDarkText}`, boxShadow: `4px 4px 0px 0px ${COLORS.successDarkText}` }} whileHover={{ x: -2, y: -2, boxShadow: `6px 6px 0px 0px ${COLORS.successDarkText}` }} whileTap={{ x: 2, y: 2, boxShadow: `1px 1px 0px 0px ${COLORS.successDarkText}` }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}>
                  Continue <ArrowRight size={18} />
                </motion.button>
                {onReview && <button onClick={onReview} className="w-full py-3 text-sm font-medium text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">Review module</button>}
              </MotionDiv>
            </div>
          </div>
        </MotionDiv>
